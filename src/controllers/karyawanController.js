...
const karyawanController = {
  ...
  async getAllKaryawan(req, res) {
    ...
    try {
      ...
      console.log('Debug - queryParams before count:', queryParams);
      ...
      const total = countResult.data[0].total;      // Get karyawan data
      const dataQuery = `
        SELECT 
          k.id_karyawan,
          k.nama,
      ...
      // Create separate array for data query params (includes pagination)
      const parsedLimit = Math.max(1, parseInt(limit) || 10);
      const parsedOffset = Math.max(0, parseInt(offset) || 0);
      const dataQueryParams = [...queryParams, parsedLimit, parsedOffset];
      
      const dataResult = await executeQuery(dataQuery, dataQueryParams);
  ...
  async getKaryawanStats(req, res) {
    try {
      ...
      const roleQuery = `        SELECT r.role_name, COUNT(*) as jumlah
        FROM karyawan k
